...
nj.deaths <- read_ucla_deaths(agencies = 'NJ')
...
# Plot NJ decedent data (wrangled to monthly numbers)
ggplot() +
    geom_line(data = nj.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
                  mutate(Date = as.Date(str_c(Year,Month,'01'), format = '%Y%B%d')),
              aes(x = Date, y = Total.Deaths)
              ) +
    geom_smooth(data = nj.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
                  mutate(Date = as.Date(str_c(Year,Month,'01'), format = '%Y%B%d')),
              aes(x = Date, y = Total.Deaths)
              )
...
nj.demographics <- harmonize_ucla_dem(agencies = 'NJ')
