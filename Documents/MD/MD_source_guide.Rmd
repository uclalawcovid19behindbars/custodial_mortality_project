...
md.deaths <- harmonize_ucla_deaths(agencies = 'MD')
...
# Plot MD decedent data (wrangled to monthly numbers)
ggplot() +
    geom_line(data = md.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
...
md.demographics <- harmonize_ucla_dem(agencies = 'MD')
