...
UCLA obtains custodial decedent data for Georgia state prisons from public records requests to the Nevada Department of Corrections.
...
nv.deaths <- harmonize_ucla_deaths(agencies = 'NV')
...
ggplot() +
    geom_line(data = nv.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
...
nv.demographics <- harmonize_ucla_dem(agencies = 'NV')
