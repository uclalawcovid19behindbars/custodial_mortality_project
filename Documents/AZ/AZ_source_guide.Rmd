...
az.deaths <- harmonize_ucla_deaths(agencies = 'AZ')
...
# Plot AZ decedent data (wrangled to monthly numbers)
ggplot() +
    geom_line(data = az.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
                  mutate(Date = as.Date(str_c(Year,Month,'01'), format = '%Y%B%d')),
              aes(x = Date, y = Total.Deaths)
              ) +
    geom_smooth(data = az.deaths %>% group_by(Year, Month) %>% summarise(Total.Deaths = n()) %>% 
                  mutate(Date = as.Date(str_c(Year,Month,'01'), format = '%Y%B%d')),
              aes(x = Date, y = Total.Deaths)
              )
...
az.demographics <- harmonize_ucla_dem(agencies = 'AZ')
